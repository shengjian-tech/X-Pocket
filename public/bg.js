      ...
      if (result.key) {
        ...
        fetch(chrome.extension.getURL("PublicResolver.json"))
          ...
          .then((jsonData) => {
            ...
            const PublicResolverController = new ethers.Contract(publicResolverAddr, abi, provider);
            try {
              PublicResolverController.functions["text(bytes32,string)"](nodeHash, key).then((resp) => {
                ...
                if (newContentUrl) {
                  var proxyConfig = {
                    mode: "pac_script",
                    pacScript: {
                      data: "function FindProxyForURL(url, host) { if (host === '" + domain + "') return 'PROXY " + newContentUrl + "'; return { cancel: false }}",
                    },
                  };
                  // 配置代理设置
